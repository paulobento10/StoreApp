<?php
//api Page https://developer.indix.com/docs#/reference
//KEY= pafgjDWrJ16ATt0UmNOnXM8FjPeeF0HA
//best buy api https://bestbuyapis.github.io/api-documentation/#retrieving-collections

//https://api.wolframalpha.com/v2/query?input=pi&appid=XXXX
//key =G884P4-U4EGUWLK4V
							...
							for (j = 0; j < 8; j++) {
								$.ajax({
									...
									success: function($convert){
										$('#wolf'+j).append('<li class="grey"> Preço maximo='+ $convert +'</li>');
									}
								});
								$.ajax({
									async: false,
									type: 'GET',
									url: 'http://labpro.dev/currency/'+$data.result.products[j].minSalePrice,
									success: function($convert){
										$('#wolf'+j).append('<li class="grey">Preço minimo='+ $convert +'</li>');
									}
								});
							}
						}
					...
					for($i=0; $i<sizeof($obtain); $i++)
					{
						...
						<div class="columns">
							<ul class="price">
								...
								<div class="box">
									<a class="pure-button" href="#popup1">Especificações</a>
								</div>
