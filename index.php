...
$app->match('/addwishlist/{pid}/{uid}', function (request $resquet,$pid,$uid) use ($app, $dbh) {
...
$app->delete('/delwishlist/{pid}/{uid}', function ($pid,$uid) use ($app, $dbh) {
...
$app->match('/indixproduct/{produto}/{offset}', function(request $request,$produto,$offset) use ($app, $dbh)
{
  $data="";
  ...
  $rows = $client->executeQuery('indixapi.products', $query);
  foreach($rows as $r)
  ...
  if($data=="")
  {
    ...
    $data=file_get_contents("https://api.indix.com/v2/summary/products?q=".$produto."&app_key=pafgjDWrJ16ATt0UmNOnXM8FjPeeF0HA&pageNumber=".$offset."&countryCode=US&pageSize=8");
    $document = ['id'=>2,'_id' => new MongoDB\BSON\ObjectId, 'search' => $data, 'produto' => $produto, 'offset'=> $offset];
    $write->insert($document);
    $result=$client->executeBulkWrite('indixapi.products', $write);
  }
  return $data;
})
->method('GET|POST');
...
$app->match('/indixsearchbrand/{marca}', function(request $request,$marca) use ($app, $dbh)
{
  ...
  $write = new MongoDB\Driver\BulkWrite;
  $data=file_get_contents('https://api.indix.com/v2/brands?q='.$marca.'&app_key=pafgjDWrJ16ATt0UmNOnXM8FjPeeF0HA');
...
$app->match('/indixsearchcategory/{categoria}', function(request $request,$categoria) use ($app, $dbh)
{
  $categoria=explode("+",$categoria);
  $categoria=implode(" ",$categoria);
  $categoria=explode("%26",$categoria);
...
$app->match('/indixsearch/product/category/{searchbar}/{categoria}/{offset}', function(request $request,$searchbar,$categoria,$offset) use ($app, $dbh)
{
  $rest=file_get_contents('http://labpro.dev/indixsearchcategory/'.$categoria);
  ...
  {
    $write = new MongoDB\Driver\BulkWrite;
    $data=file_get_contents('https://api.indix.com/v2/summary/products?q='.$searchbar.'&app_key=pafgjDWrJ16ATt0UmNOnXM8FjPeeF0HA&categoryId='.$rest.'&pageNumber='.$offset.'&countryCode=US&pageSize=8');
...
$app->match('/indixsearch/product/brand/{searchbar}/{brand}/{offset}', function(request $request,$searchbar,$brand,$offset) use ($app, $dbh)
{
  $idbrand=file_get_contents('http://labpro.dev/indixsearchbrand/'.$brand);
  $data="";
  $client= new MongoDB\Driver\Manager(); // connect
  ...
  {
    $write = new MongoDB\Driver\BulkWrite;
    $data=file_get_contents('https://api.indix.com/v2/summary/products?q='.$searchbar.'&app_key=pafgjDWrJ16ATt0UmNOnXM8FjPeeF0HA&brandId='.$idbrand.'&pageNumber='.$offset.'&countryCode=US&pageSize=8');
...
$app->match('/indixsearch/product/brand/category/{searchbar}/{brand}/{category}/{offset}', function(request $request,$searchbar,$brand,$category,$offset) use ($app, $dbh)
{
  $idbrand=file_get_contents('http://labpro.dev/indixsearchbrand/'.$brand);
  $idcat=file_get_contents('http://labpro.dev/indixsearchcategory/'.$category);
  $data="";
  $client= new MongoDB\Driver\Manager(); // connect
  ...
  {
    $write = new MongoDB\Driver\BulkWrite;
    $data=file_get_contents('https://api.indix.com/v2/summary/products?q='.$searchbar.'&app_key=pafgjDWrJ16ATt0UmNOnXM8FjPeeF0HA&categoryId='.$idcat.'&brandId='.$idbrand.'&pageNumber='.$offset.'&countryCode=US&pageSize=8');
...
$app->match('/indixsearch/brand/category/{brand}/{category}/{offset}', function(request $request,$brand,$category,$offset) use ($app, $dbh)
{
  $idbrand=file_get_contents('http://labpro.dev/indixsearchbrand/'.$brand);
  $idcat=file_get_contents('http://labpro.dev/indixsearchcategory/'.$category);
  $data="";
  ...
  {
    $write = new MongoDB\Driver\BulkWrite;
    $data=file_get_contents('https://api.indix.com/v2/summary/products?app_key=pafgjDWrJ16ATt0UmNOnXM8FjPeeF0HA&categoryId='.$idcat.'&brandId='.$idbrand.'&pageNumber='.$offset.'&countryCode=US&pageSize=8');
...
$app->match('/indixcategoriaprodutct/{categoria}', function(request $request,$categoria) use ($app, $dbh)
{
  $categoria=explode("+",$categoria);
  $categoria=implode(" ",$categoria);
  ...
  $write = new MongoDB\Driver\BulkWrite;
  $data=file_get_contents('https://api.indix.com/v2/categories?app_key=pafgjDWrJ16ATt0UmNOnXM8FjPeeF0HA&pageSize=8');
...
$app->match('/indixproductbrand/{brand}/{offset}', function(request $request,$brand,$offset) use ($app, $dbh)
{
  $id= file_get_contents('http://labpro.dev/indixsearchbrand/'.$brand);
  $data="";
  $client= new MongoDB\Driver\Manager(); // connect
  ...
  {
    $write = new MongoDB\Driver\BulkWrite;
    $data=file_get_contents('https://api.indix.com/v2/summary/products?app_key=pafgjDWrJ16ATt0UmNOnXM8FjPeeF0HA&&brandId='.$id.'&pageNumber='.$offset.'&countryCode=US&pageSize=8');
...
$app->match('/indixproductcategory/{categoria}/{offset}', function(request $request,$categoria,$offset) use ($app, $dbh)
{

  $data="";
  $idcat= file_get_contents('http://labpro.dev/indixcategoriaprodutct/'.$categoria);
  $client= new MongoDB\Driver\Manager(); // connect
  ...
  {
    $write = new MongoDB\Driver\BulkWrite;
    $data=file_get_contents('https://api.indix.com/v2/summary/products?app_key=pafgjDWrJ16ATt0UmNOnXM8FjPeeF0HA&categoryId='.$idcat.'&pageNumber='.$offset.'&countryCode=US&pageSize=8');
...
$app->match('/indixdefault/{offset}', function(request $resquest,$offset) use ($app, $dbh)
{
  $data="";
  $client= new MongoDB\Driver\Manager(); // connect
  ...
  {
    $write = new MongoDB\Driver\BulkWrite;
    $data = file_get_contents("https://api.indix.com/v2/summary/products?q=apple&app_key=pafgjDWrJ16ATt0UmNOnXM8FjPeeF0HA&brands?q=apple&storeId=24&categoryId=10161&pageNumber=".$offset."&countryCode=US&pageSize=8");
...
$app->put('/editindixdefault/{offset}', function(request $request,$offset) use ($app, $dbh)
{
...
$app->put('/editindixproduct/{produto}/{offset}', function(request $request,$produto,$offset) use ($app, $dbh)
{
  $data = $request->getContent();
  $client= new MongoDB\Driver\Manager();
  $write = new MongoDB\Driver\BulkWrite;
...
$app->put('/editindixcategoria/{categoria}/{offset}', function(request $request,$categoria,$offset) use ($app, $dbh)
{
  $categoria=file_get_contents('http://labpro.dev/indixsearchcategory/'.$categoria);
  $data = $request->getContent();
  $client= new MongoDB\Driver\Manager();
  $write = new MongoDB\Driver\BulkWrite;
  $write->update(
...
$app->put('/editindixmarca/{marca}/{offset}', function(request $request,$marca,$offset) use ($app, $dbh)
{
  $marca=file_get_contents('http://labpro.dev/indixsearchbrand/'.$marca);
  $data = $request->getContent();
  $client= new MongoDB\Driver\Manager();
  $write = new MongoDB\Driver\BulkWrite;
  $write->update(
...
$app->put('/editindixprodutocategoria/{produto}/{categoria}/{offset}', function(request $request,$produto,$categoria,$offset) use ($app, $dbh)
{
  $categoria=file_get_contents('http://labpro.dev/indixsearchcategory/'.$categoria);
  $data = $request->getContent();
  $client= new MongoDB\Driver\Manager();
...
$app->put('/editindixprodutomarca/{produto}/{marca}/{offset}', function(request $request,$produto,$marca,$offset) use ($app, $dbh)
{
  $marca=file_get_contents('http://labpro.dev/indixsearchbrand/'.$marca);
  $data = $request->getContent();
  $client= new MongoDB\Driver\Manager();
...
$app->put('/editindixcategoriamarca/{categoria}/{marca}/{offset}', function(request $request,$categoria,$marca,$offset) use ($app, $dbh)
{
  $categoria=file_get_contents('http://labpro.dev/indixsearchcategory/'.$categoria);
  $marca=file_get_contents('http://labpro.dev/indixsearchbrand/'.$marca);
  $data = $request->getContent();
...
$app->put('/editindixcategoriamarcaproduto/{categoria}/{marca}/{produto}/{offset}', function(request $request,$categoria,$marca,$produto,$offset) use ($app, $dbh)
{
  $categoria=file_get_contents('http://labpro.dev/indixsearchcategory/'.$categoria);
  $marca=file_get_contents('http://labpro.dev/indixsearchbrand/'.$marca);
  $data = $request->getContent();
